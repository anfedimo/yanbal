...
@ManagedBean(name = "otroBean")
@ViewScoped
public class OtroBean {

	private static Logger logger = Logger.getLogger(OtroBean.class);
	@EJB
	...
	public void init() {
		try {
			...
			tipificaciones = tipificacionEJB.buscarTipificaciones(GrupoTIpificacionEnum.CASOS_ESPECIALES.getIndex());
	...
	public void guardarOtro() {
		if (idTipificacion == null || idTipificacion == 0 || StringUtils.isBlank(Observacion)) {
			...
			RequestContext.getCurrentInstance().update("frmOtros:dlOtros");
			RequestContext.getCurrentInstance().execute("PF('dlOtros').show();");
			return;
		...
		try {
			...
			// gestion.setId(clienteBean.getIdgestion() != null ?
			// Integer.parseInt(clienteBean.getIdgestion()) : 0);
